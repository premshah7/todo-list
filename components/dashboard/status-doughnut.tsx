    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-popover border border-border p-2 rounded-lg shadow-xl">
                <p className="text-foreground text-xs font-medium">{payload[0].name}</p>
...
export function StatusDoughnut({ data = [] }: StatusDoughnutProps) {
    ...
    if (total === 0) {
        return (
            <Card className="bg-card border-border h-full min-h-[300px] flex flex-col items-center justify-center text-muted-foreground p-6 shadow-sm">
                    ...
                    <PieChart>
                        ...
                        <Legend
                            ...
                            formatter={(value, entry: any) => (
                                <span className="text-xs text-muted-foreground ml-1 font-medium">{value} ({entry.payload.value})</span>
                            )}
                        />
                    </PieChart>
                </ResponsiveContainer>
